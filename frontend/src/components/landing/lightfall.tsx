...
export interface LightfallProps {
  ...
  mixBlendMode?: string;
  /** Cap frames per second (default 30). */
  maxFps?: number;
}
...
const prepColors = (input?: string[]) => {
  const fallback = ["#A6C8FF", "#5227FF", "#FF9FFC"];
  const base = (input && input.length ? input : fallback).slice(0, MAX_COLORS);
  const count = base.length;
  const arr: RGB[] = Array.from({ length: MAX_COLORS }, (_, i) =>
    hexToRGB(base[Math.min(i, base.length - 1)] ?? fallback[0]!),
  );
  const avg: RGB = [0, 0, 0];
...
const Lightfall: FC<LightfallProps> = ({
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      if (!pausedRef.current && meshRef.current) {
        try {
